...
The Linux x86_64 fitted-model reference was generated by the pinned Ubuntu
24.04 CI runtime after that review. A second clean Ubuntu run must match the
committed candidate before the reference is accepted; a macOS baseline is not
substituted for it.
