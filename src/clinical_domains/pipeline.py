...
def run_reproduction(config_path: str | Path, output_dir: str | Path) -> dict[str, Path]:
    config = load_yaml(config_path)
    seed = config.get("modeling", {}).get("seed", 20260724)
    set_global_seed(seed)
    landmark_hours = config.get("study", {}).get("landmark_hours", 24)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    data = GenericEHRAdapter.from_config(config_path).extract_all()
